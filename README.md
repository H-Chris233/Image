...
| `RECHARGE_URL` | `SUB2API_AUTH_BASE_URL` | 外部充值页面地址；未配置时默认使用 Sub2API 站点地址 |

充值入口会直接打开 `RECHARGE_URL`。如果未显式配置，AetherGenix 会使用 `SUB2API_AUTH_BASE_URL` 作为充值站点地址。管理员也可以在系统设置里覆盖充值中心地址。
