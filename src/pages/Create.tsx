...
import { ArrowUp, ChevronDown, Clock3, ImagePlus, SlidersHorizontal, Sparkles, X } from 'lucide-react';
import {
  editImage,
  generateImage,
  optimizePrompt,
...
import { Button, FileInput, IconButton, Pressable, TextareaControl } from '../components/design-system';
  ...
  const renderActionButtons = (className = '') => (
    <div className={`grid grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] gap-2 ${className}`}>
      <Button
        ...
        iconStart={<Sparkles aria-hidden="true" size={13} />}
      ...
      <Button
        ...
        disabled={loading}
        loading={loading}
        iconStart={<ArrowUp aria-hidden="true" size={15} />}
        className="min-h-11 min-w-0 px-3 text-sm shadow-[0_0_0_1px_rgba(255,255,255,0.18),0_16px_36px_rgba(240,237,232,0.14)]"
      >
      ...
      <main className="mx-auto flex w-full max-w-3xl flex-col gap-3 lg:min-h-[calc(100vh-144px)] lg:justify-center">
        ...
        <section className="overflow-hidden rounded-lg border border-[rgba(227,255,116,0.16)] bg-[#1a1917] shadow-[0_22px_90px_rgba(0,0,0,0.62),0_0_0_1px_rgba(255,255,255,0.03)]">
          ...
          >
            <div className="flex gap-3">
              <IconButton
                ...
                label={t('create_add_reference')}
                icon={<ImagePlus aria-hidden="true" size={18} className="text-[#d8d3cc]" />}
                onClick={openReferenceFilePicker}
              />
              <TextareaControl
                ref={textareaRef}
                unstyled
                className="min-h-[188px] min-w-0 flex-1 resize-none bg-transparent text-base leading-7 text-[#f0ede8] outline-none placeholder:text-[#5a5650] sm:min-h-[220px]"
                  ...
                  {selectedReferencePreviews.map((ref) => (
                    ...
                    >
                      ...
                      <IconButton
                        ...
                        label={t('create_remove_reference')}
          ...
          <div className="border-t border-white/[0.06] bg-[#141310] px-3 py-3 sm:px-4">
            ...
            {renderActionButtons('mt-3 hidden lg:ml-auto lg:grid lg:max-w-[360px]')}
          </div>
        </section>
      </main>

      <div className="fixed inset-x-0 bottom-16 z-40 border-t border-white/[0.08] bg-[#111110]/95 px-3 py-3 shadow-[0_-18px_48px_rgba(0,0,0,0.42)] backdrop-blur-xl sm:px-4 lg:hidden">
