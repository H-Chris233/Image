...
└─ step=browse
   └─ SceneBrowser(onPick)
      ...
      ├─ scenes = scenesOf(activeCustomer)
      ├─ 左栏：CUSTOMERS
      │  └─ 点击 → setActiveCustomer()
      └─ 右栏：SceneCard[]
         └─ 点击 → onPick(scene)
            └─ dispatch(select_template) → step=upload

src/components/ecommerce/sceneCatalog.ts
├─ CUSTOMERS        ← 生意类型；分类轴待重构
├─ SCENE_CATALOG    ← 场景 + prompt + sampleImage
└─ scenesOf(key)    ← 按 customerKey 过滤
```
